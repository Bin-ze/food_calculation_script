...
class Cumpute_pred_acc:
    ...
    def __init__(self, pred_path, label_path, iou_thres=0.3, result_sava_path='./'):
        """
    ...
    def compute_single_img(self, pred_txt, label_txt):
        ...
        pred_result = self.norm_yolox(pred_result)
        ...
        for instance in label_result:
            ...
            # update **********************************************************
            bbox_iou = self.bbox_iou(bbox_instance, pred_result[:, 1:])
    ...
    def format_result(self):
        ...
        logging.info('Missing detection rate: {:.4f}'.format((len(set(self.miss))) / self.total_img_nums))
        logging.info('False detection rate: {:.4f}'.format((len(set(self.classify_error))) / self.total_img_nums))
    ...
    @staticmethod
    def norm_yolox(box, wh=(1000, 720)):
    ...
    @staticmethod
    def plot_bbox(txt_path, img_path):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label_path', type=str, default='/mnt/data/guozebin/coco/labels/test',help='gt txt save path')
    parser.add_argument('--pred_path', type=str, default='/mnt/data/guozebin/object_detection/CenterNet2/infer_result/labels',help='pred result txt save path')
    parser.add_argument('--result_save_path', type=str, default='./centernet.txt', help='acc result and error and miss result save path')
