...
def main(args):
    ...
    print("using {} images for training, {} images fot validation.".format(train_num,
                                                                           val_num))
    net = load_classifier(name='resnet18', n=len(cla_dict))
    net.to(device)
    LR = args.LR
    ...
    for epoch in range(epochs):
        ...
        for step, data in enumerate(train_bar):
            ...
            # one hot
            label_onehot = torch.zeros(len(cla_dict), len(labels))
            labels = label_onehot.scatter(0, labels.unsqueeze(dim=0), 1).T
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', default='./model_celoss.pth', help='weight save path')
